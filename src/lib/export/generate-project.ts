...
export function generateProject(
  ...
  const files: ExportFile[] = [
    ...
    { path: "README.md", content: generateReadme(spec.businessName, includeBadge) },
  ];
...
function generateHTML(
  ...
  const badgeHtml = includeBadge
    ? `
<!-- Built with EasyWebsiteBuild -->
<a href="https://easywebsitebuild.com?utm_source=export&utm_medium=badge&utm_campaign=viral" target="_blank" rel="noopener noreferrer" style="position:fixed;bottom:16px;right:16px;z-index:9999;display:flex;align-items:center;gap:8px;padding:8px 14px;background-color:rgba(13,14,20,0.85);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border-radius:10px;border:1px solid rgba(255,255,255,0.08);text-decoration:none;box-shadow:0 2px 12px rgba(0,0,0,0.3);transition:all 0.2s ease;" onmouseover="this.style.boxShadow='0 4px 20px rgba(232,168,73,0.2)';this.style.transform='translateY(-1px)';this.querySelector('.ewb-text').style.color='#e8a849'" onmouseout="this.style.boxShadow='0 2px 12px rgba(0,0,0,0.3)';this.style.transform='none';this.querySelector('.ewb-text').style.color='rgba(255,255,255,0.7)'"><span style="display:flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:6px;background:linear-gradient(135deg,#e8a849,#3ecfb4);font-weight:800;font-size:13px;color:#0a0b0f;line-height:1;flex-shrink:0;">E</span><span class="ewb-text" style="font-size:12px;font-weight:500;color:rgba(255,255,255,0.7);white-space:nowrap;transition:color 0.2s ease;">Built with EasyWebsiteBuild</span></a>
`
    : "";
...
function generateReadme(businessName: string, includeBadge: boolean = true): string {
  const badgeNote = includeBadge
    ? `

## Badge

This site includes a "Built with EasyWebsiteBuild" badge.
Upgrade to Pro at https://easywebsitebuild.com/pricing to remove it.
`
    : "";

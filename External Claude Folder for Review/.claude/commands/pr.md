...
### Step 2: Ensure Branch is Tracked and Pushed

First, ensure the branch is tracked by Graphite:

```bash
gt branch info 2>&1 || gt track --parent main
```

If there are unpushed commits, sync with Graphite:
...
### Step 5: Create PR with Graphite

Use Graphite to create/update the PR (enables AI code reviews):
...
- Create a PR with Graphite tracking
- Enable AI code reviews automatically

If you need to add a detailed body, use:

```bash
gt submit -m "<title>" --body "$(cat <<'EOF'
<generated body>
EOF
)"
```

**Fallback** (if Graphite fails):

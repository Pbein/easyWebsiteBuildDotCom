...
2. **Analyze changes to determine**:
   ...
   - **Scope**: Primary folder/domain affected (e.g., `analytics`, `billing`, `sms`)
...
### Step 6: Push with Graphite

After successful commit, use Graphite for AI code reviews:
...
- Create/update a PR with Graphite tracking
- Enable AI code reviews on the PR

If Graphite submit fails, fall back to git:
...
| ------------------- | ------------------------------------- |
| ESLint errors       | `bun run lint --fix`                  |
| Prettier formatting | `bunx prettier --write .`             |
